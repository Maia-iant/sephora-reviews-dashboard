...
def load_data():
    ...
    file_path = base_dir / "Data" / "raw" / "reviews_sample_10k.csv"
...
def safe_metric(value, decimals=2):
    if pd.isna(value):
        return "N/A"
...
with tab1:
    ...
    if "rating" in df_filtered.columns and df_filtered["rating"].notna().any():
        col3.metric("Note moyenne", safe_metric(df_filtered["rating"].mean()))
    else:
        col3.metric("Note moyenne", "N/A")

    if "is_recommended_clean" in df_filtered.columns:
        reco_series = df_filtered["is_recommended_clean"].dropna()
...
with tab2:
    ...
    if "price_usd" in df_filtered.columns and df_filtered["price_usd"].notna().any():
        k2.metric("Prix moyen USD", safe_metric(df_filtered["price_usd"].mean()))
    else:
        k2.metric("Prix moyen USD", "N/A")

    if "rating" in df_filtered.columns and df_filtered["rating"].notna().any():
        k3.metric("Note moyenne", safe_metric(df_filtered["rating"].mean()))
    else:
        k3.metric("Note moyenne", "N/A")

    g3, g4 = st.columns(2)

    if "skin_type_clean" in df_filtered.columns and df_filtered["skin_type_clean"].notna().any():
        with g3:
            skin_counts = (
    ...
    if "skin_type_clean" in df_filtered.columns and "rating" in df_filtered.columns:
        ...
        tmp = tmp[tmp["skin_type_clean"].astype(str).str.strip() != ""]
        if not tmp.empty:
            with g4:
                avg_by_skin = (
    ...
    if "skin_type_clean" in df_filtered.columns and "price_usd" in df_filtered.columns:
        ...
        tmp_price = tmp_price[tmp_price["skin_type_clean"].astype(str).str.strip() != ""]
        ...
        if not tmp_price.empty:
            ...
            if "rating" in tmp_price.columns:
                agg_dict["note_moyenne"] = ("rating", "mean")
            ...
            st.info("Aucune donnée exploitable pour le prix par type de peau avec les filtres actuels.")
...
with tab3:
    ...
    if "review_text" in df_filtered.columns:
        ...
        sentiment_results = text_df["review_text"].fillna("").astype(str).apply(simple_sentiment)
        ...
        t1.metric("Longueur moyenne", safe_metric(text_df["review_length"].mean(), 1))
        t2.metric("Nombre moyen de mots", safe_metric(text_df["word_count"].mean(), 1))
        t3.metric("Avis non vides", int((text_df["review_text_clean"] != "").sum()))
        t4.metric("Score moyen", safe_metric(text_df["sentiment_score"].mean(), 2))
        ...
        st.info("La colonne review_text est absente du fichier actuel.")
...
with st.expander("Diagnostic colonnes"):
    ...
    if "skin_type_clean" in df.columns:
        st.write("Valeurs skin_type_clean :", df["skin_type_clean"].dropna().astype(str).str.strip().unique().tolist()[:50])
